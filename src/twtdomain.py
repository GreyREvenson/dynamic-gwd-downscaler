...
def set_domain(*,
    ...
    if verbose:
        logger.info('calling set_domain')
    if fname_domain is None:
        raise ValueError('_set_domain missing required argument fname_domain')
    if not os.path.isfile(fname_domain) or overwrite:
        if domain_hucid is not None:
            ...
            raise Exception('_set_domain could not set domain from arguments')
        ...
        if verbose:
            logger.info(f'using existing domain {fname_domain}')
...
def set_domain_buf(*,
    ...
    overwrite: bool = False):
    if verbose:
        logger.info('calling set_domain_buf')
    if not os.path.isfile(fname_domain_buf) or overwrite:
        if verbose:
            logger.info(f'creating domain buffer {fname_domain_buf} with buffer distance {buf_dist_m} m')
        ...
        if verbose:
            logger.info(f'found existing domain buffer file {fname_domain_buf}')
...
def _set_domain_byhucid(*,
    ...
    verbose: bool = False):
    if verbose:
        logger.info('_set_domain_byhucid')
...
def _set_domain_bybbox(*,
    ...
    verbose: bool = False):
    if verbose:
        logger.info('_set_domain_bybbox')
...
def _set_domain_bylatlonandhuclvl(*,
    ...
    verbose: bool = False):
    if verbose:
        logger.info('_set_domain_bylatlonandhuclvl')
    ...
    if (not isinstance(domain_latlon, list)
        or not all(isinstance(v, float) for v in domain_latlon)
        or len(domain_latlon) != 2):
        raise ValueError('_set_domain_bylatlonandhuclvl invalid domain_latlon')
...
def get_conus1_hucs(*,
    ...
    verbose: bool = False):
    if verbose:
        logger.info('calling get_conus1_hucs')
    if not os.path.isfile(fname_domain_hucs):
        fname_wb_full_temp = os.path.join(os.path.dirname(fname_domain_hucs),
        ...
        # Fix: read the existing domain_hucs file, not the temp wb_full file
