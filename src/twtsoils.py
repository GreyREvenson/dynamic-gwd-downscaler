...
def break_soil_texture(*,
    ...
    overwrite=False):
    if verbose:
        logger.info('calling break_soil_texture')
    if not os.path.isfile(fname_texture_child) or overwrite:
        ...
        if verbose:
            logger.info(f'found existing soil texture file {fname_texture_child}')
...
async def download_soil_texture(*,
    ...
    overwrite=False):
    if verbose:
        logger.info('calling set_soil_texture')
    if not os.path.isfile(fname_texture) or overwrite:
        if verbose:
            logger.info(f'using soildb to download soil texture and saving to {fname_texture}')
        ...
        dom_comps = comps.loc[comps.groupby('mukey')['comppct_r'].idxmax()]  # dominant component per map unit
        ...
        sand = chorizons.groupby('cokey')[['sandtotal_r', 'depth_range']].apply(
            lambda x: (x['sandtotal_r'] * x['depth_range']).sum() / x['depth_range'].sum()
        ).reset_index(name='weighted_sandtotal_r')
        silt = chorizons.groupby('cokey')[['silttotal_r', 'depth_range']].apply(
            lambda x: (x['silttotal_r'] * x['depth_range']).sum() / x['depth_range'].sum()
        ).reset_index(name='weighted_silttotal_r')
        clay = chorizons.groupby('cokey')[['claytotal_r', 'depth_range']].apply(
            lambda x: (x['claytotal_r'] * x['depth_range']).sum() / x['depth_range'].sum()
        ).reset_index(name='weighted_claytotal_r')
        texture = sand.merge(silt, on='cokey').merge(clay, on='cokey')
        def calc_texture(row):
            try:
                sand_val = float(row['weighted_sandtotal_r'])
            except Exception:
                return 'None'
            try:
                clay_val = float(row['weighted_claytotal_r'])
            except Exception:
                return 'None'
            return soiltexture.getTexture(sand_val, clay_val)
        ...
        if verbose:
            logger.info(f'found existing soil texture file {fname_texture}')
...
def set_soil_transmissivity(*,
    ...
    overwrite=False):
    if verbose:
        logger.info('calling set_soil_transmissivity')
    if not os.path.isfile(fname_transmissivity) or overwrite:
        if verbose:
            logger.info(f'creating {fname_transmissivity} from {fname_texture}')
        ...
        def calc_f(row):
            texture_key = str(row.get('texture', '')).lower()
            return dt_transmissivity.get(texture_key, float(np.mean(list(dt_transmissivity.values()))))
        soil_texture['f'] = soil_texture.apply(calc_f, axis=1)
        # Fix: chunks typo
        with rioxarray.open_rasterio(fname_dem, masked=True, chunks='auto') as riox_ds_dem:
            ...
            percnan = (nancount / tot) * 100.0
            nanmean = float(soil_texture_grid.mean(skipna=True).f)
            if verbose and nancount > 0:
                logger.warning(f'{nancount} of {tot} cells (~{percnan:.2f}% of domain) had nan transmissivity - filling with grid mean value {nanmean}')
        ...
        if verbose:
            logger.info(f'using existing transmissivity data {fname_transmissivity}')