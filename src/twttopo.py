...
async def download_dem(*,
    ...
    if verbose:
        logger.info('calling download_dem')
    if not os.path.isfile(fname_dem) or overwrite:
        ...
        for k in avail.keys():
            try:
                vals.append(int(k.replace('m', '')))
            except Exception:
                pass
        if len(vals) == 0:
            raise ValueError('download_dem 3dep could not find dem resolution')
        rez = min(vals)
        if dem_rez in vals:
            rez = dem_rez
        if verbose:
            logger.info(f'downloading {str(rez)}m DEM to {fname_dem}')
        for attempt in range(1, n_retries + 1):
            ...
            try:
                ...
                if verbose:
                    logger.warning(f"DEM download timed out (attempt {attempt}/{n_retries}) at {rez} m")
        ...
        if verbose:
            logger.info(f'found existing dem {fname_dem}')
...
def break_dem(*,
    ...
    if verbose:
        logger.info('calling break_dem')
    if not os.path.isfile(fname_dem_child) or overwrite:
        if verbose:
            logger.info(f'creating {fname_dem_child} from {fname_dem_parent}')
        gdal.UseExceptions()
        src_ds = gdal.Open(fname_dem_parent, gdal.GA_ReadOnly)
        if src_ds is None:
            raise RuntimeError(f"Failed to open VRT: {fname_dem_parent}")
        warp_kwargs = {
            "format": "GTiff",
        ...
        if verbose:
            logger.info(f'found {fname_dem_child}')
...
def breach_dem(*,
    ...
    if verbose:
        logger.info('calling breach_dem')
    if not os.path.isfile(fname_dem_breached) or overwrite:
        if verbose:
            logger.info(f'using whitebox to breach dem and writing to {fname_dem_breached}')
        wbt = whitebox.WhiteboxTools()
        wbt.set_compress_rasters(True)

        root, ext = os.path.splitext(fname_dem)
        fname_filled = f"{root}_fill{ext}"
        ...
        if verbose:
            logger.info(f'found existing breached dem {fname_dem_breached}')
...
def set_flow_acc(*,
    ...
    if verbose:
        logger.info('calling set_flow_acc')
    if not os.path.isfile(fname_facc_ncells) or overwrite:
        if verbose:
            logger.info(f'using whitebox to calculate flow accumulation (n cells), writing to {fname_facc_ncells}')
        wbt = whitebox.WhiteboxTools()
        wbt.set_compress_rasters(True)
        ...
        if verbose:
            logger.info(f'using existing flow accumulation (ncells) file {fname_facc_ncells}')
    if not os.path.isfile(fname_facc_sca) or overwrite:
        if verbose:
            logger.info(f'using whitebox to calculate flow accumulation (sca), writing to {fname_facc_sca}')
        wbt = whitebox.WhiteboxTools()
        wbt.set_compress_rasters(True)
        wbt.d_inf_flow_accumulation(i=fname_dem_breached,
        ...
        if verbose:
            logger.info(f'using existing flow accumulation (sca) file {fname_facc_sca}')

def calc_stream_mask(*,
    verbose: bool = False,
    overwrite: bool = False,
    fname_facc_ncells: str = None,
    facc_threshold_ncells: int = None,
    fname_facc_sca: str = None,
    facc_threshold_sca: float = None,
    fname_strm_mask: str = None):

    if verbose:
        logger.info('calling calc_stream_mask')
    if not os.path.isfile(fname_strm_mask) or overwrite:
        if os.path.isfile(fname_facc_ncells):
            if verbose:
                logger.info(f'setting stream mask using fname_facc_ncells {fname_facc_ncells} and facc_threshold_ncells {facc_threshold_ncells}')
            wbt = whitebox.WhiteboxTools()
            wbt.set_compress_rasters(True)
            ...
            if verbose:
                logger.info(f'setting stream mask using fname_facc_sca {fname_facc_sca} and facc_threshold_sca {facc_threshold_sca}')
            wbt = whitebox.WhiteboxTools()
            wbt.set_compress_rasters(True)
        ...
        if verbose:
            logger.info(f'using existing stream mask {fname_strm_mask}')
...
def calc_slope(*,
    ...
    if verbose:
        logger.info('calling calc_slope')
    if not os.path.isfile(fname_slope) or overwrite:
        ...
        if verbose:
            logger.info(f'using existing slope file {fname_slope}')
...
def calc_twi(*,
    ...
    if verbose:
        logger.info('calling calc_twi')
    if not os.path.isfile(fname_twi) or overwrite:
        if verbose:
            logger.info(f'using whitebox workflows to calculate {fname_twi}')
        wbt = whitebox.WhiteboxTools()
        wbt.set_compress_rasters(True)
        ...
        if verbose:
            logger.info(f'found existing TWI file {fname_twi}')
...
def calc_twi_mean(*,
    ...
    """
    if verbose:
        logger.info('calling calc_twi_mean')
    ...
    if os.path.isfile(fname_twi_mean) and not overwrite:
        if verbose:
            logger.info(f'found existing mean TWI file {fname_twi_mean}')
        return
    ...
    if verbose:
        logger.info(f'calculating mean TWI per WTD cell and saving on TWI grid -> {fname_twi_mean}')
    ...
    with rioxarray.open_rasterio(fname_twi, masked=True) as riox_ds_twi, \
        ...
        if verbose:
            logger.info(f'wrote {fname_twi_mean}')
...
def set_domain_mask(*,
    ...
    if verbose:
        logger.info('calling set_domain_mask')
    if not os.path.isfile(fname_domain_mask) or overwrite:
        if verbose:
            logger.info(f'creating {fname_domain_mask}')
        ...
        if verbose:
            logger.info(f'using existing domain mask {fname_domain_mask}')
...
def set_streams(*,
    ...
    if verbose:
        logger.info('calling set_streams')
    if not os.path.isfile(fname_streams) or overwrite:
        if verbose:
            logger.info(f'using pynhd to download NHDPlusHR flowlines - saving to {fname_streams}')
        ...
        if verbose:
            logger.info(f'using existing NHD line file {fname_streams}')